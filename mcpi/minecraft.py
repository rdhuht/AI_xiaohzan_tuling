...
class Minecraft:
    ...
    def getBlock(self, *args):
        ...
        """根据block的位置(x,y,z)返回得到该block的ID"""
        """data = mc.getBlock(x, y - 1, z) 
           print(data)
           检测玩家脚下的block的ID，如果是站在羊毛上则，data就是 35
        """
    ...
    def getBlockWithData(self, *args):
        ...
        """data = mc.getBlockWithData(x, y - 1, z)
           print(data)
           检测玩家脚下的block的信息，如果是站在橙色羊毛上面，data 是 Block(35, 1) 第一个参数是block的类型，第二个参数是颜色          
        """
    ...
    def getBlocks(self, *args):
        ...
        """blocks使用for遍历循环就可以得到每一个方块"""
        s = self.conn.sendReceive(b"world.getBlocks", intFloor(args))
        return map(int, s.split(","))

    def setBlock(self, *args):
        """Set block (x,y,z,id,[data])"""
        """在指定的(x, y, z)坐标放置id类型的block，颜色是[data](整数型数据)"""
        """mc.setBlock(1000, 29, 1000, 35, 1)
           在(1000, 29, 1000)的位置放置一个ID是35的羊毛砖块，颜色是1橙色
        """
        self.conn.send(b"world.setBlock", intFloor(args))

    def setBlocks(self, *args):
        """Set a cuboid of blocks (x0,y0,z0,x1,y1,z1,id,[data])"""
        """以(x0,y0,z0)和(x1,y1,z1)为立方体的写对角线的顶点，建造一个立方体"""
    ...
    # 2: 47.105.47.99
    # 3: 47.104.14.66
    # 本地多人，192.168.0.43 (不同的wifi网络这里的IP是不同的，每次换网，请查看Wi-Fi的IP)
